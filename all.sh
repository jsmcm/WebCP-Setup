...
yum install php70w php70w-cli php70w-common php70w-gd php70w-mbstring php70w-mcrypt php70w-xml php70w-xmlrpc php70w-soap php70w-imap php70w-fpm php70w-opcache -y
...
mkdir /var/log/webcp
chown apache.apache /var/log/webcp -R
chmod 755 /var/log/webcp -R
...
yum install mysql proftpd-mysql mysql-connector-odbc mysql-devel mysql-libs dovecot-mysql php70w-mysql mysql-server -y

systemctl stop mysqld

cd /etc/
rm -fr my.cnf
wget http://api.webcp.pw/downloads/2.0.0/my.cnf
...
touch whitelist
chown apache.apache whitelist
chmod 644 whitelist

touch system_filter
chmod 644 system_filter

openssl req -batch -nodes -x509 -newkey rsa:2048 -keyout privkey.key -out cert.crt -days 365
...
gpasswd -a exim clamupdate
gpasswd -a exim  clamscan
gpasswd -a mail clamupdate
gpasswd -a mail clamscan

cat /usr/lib/systemd/system/clamd.service | sed "s/%i/clamd/g" > /usr/lib/systemd/system/clamd.servicetemp
rm -fr /usr/lib/systemd/system/clamd.service
mv /usr/lib/systemd/system/clamd.servicetemp /usr/lib/systemd/system/clamd.service
